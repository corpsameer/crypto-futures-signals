...
@section('content')
    ...
    <div class="row g-4 mt-1">
        <div class="col-12 col-xl-6">
            <div class="card metric-card h-100">
                <div class="card-header bg-white"><h2 class="h5 mb-0">Recent Signals</h2></div>
                <div class="card-body p-0">
                    @if (($recentSignals ?? collect())->isNotEmpty())
                        <div class="table-responsive"><table class="table table-sm table-hover align-middle mb-0"><thead class="table-light"><tr><th>Signal</th><th>Status</th><th>Entry</th></tr></thead><tbody>
                            @foreach ($recentSignals as $signal)
                                <tr><td><a href="{{ route('cryptofuturesignals.trade-signals.show', $signal) }}" class="fw-semibold text-decoration-none">{{ $signal->symbol ?: 'N/A' }}</a><div class="mt-1"><x-signal-source-badge :source="$signal" /></div></td><td>{{ $signal->status ?: 'N/A' }}</td><td>{{ $signal->entry_display }}</td></tr>
                            @endforeach
                        </tbody></table></div>
                    @else
                        <div class="p-4 text-muted">No recent signals yet.</div>
                    @endif
                </div>
            </div>
        </div>
        <div class="col-12 col-xl-6">
            <div class="card metric-card h-100">
                <div class="card-header bg-white"><h2 class="h5 mb-0">Recent Simulated Trades</h2></div>
                <div class="card-body p-0">
                    @if (($recentTrades ?? collect())->isNotEmpty())
                        <div class="table-responsive"><table class="table table-sm table-hover align-middle mb-0"><thead class="table-light"><tr><th>Trade</th><th>Status</th><th>Entry Price</th></tr></thead><tbody>
                            @foreach ($recentTrades as $trade)
                                <tr><td>@if ($trade->tradeSignal)<a href="{{ route('cryptofuturesignals.trade-signals.show', $trade->tradeSignal) }}" class="fw-semibold text-decoration-none">{{ $trade->symbol ?: 'N/A' }}</a>@else<span class="fw-semibold">{{ $trade->symbol ?: 'N/A' }}</span>@endif<div class="mt-1"><x-signal-source-badge :source="$trade->tradeSignal?->signal_source ?? 'unknown'" /></div></td><td>{{ $trade->status ?: 'N/A' }}</td><td>{{ $trade->entry_price ?? 'N/A' }}</td></tr>
                            @endforeach
                        </tbody></table></div>
                    @else
                        <div class="p-4 text-muted">No recent simulated trades yet.</div>
                    @endif
                </div>
            </div>
        </div>
    ...
    <div class="alert alert-info mt-4 mb-0 d-flex flex-wrap justify-content-between align-items-center gap-2">
        <span>Dashboard summaries are based on saved structured signals, simulated trades, and tracking events generated by the Python monitor.</span>
        <a href="{{ route('cryptofuturesignals.logs.index') }}" class="btn btn-outline-primary btn-sm">System Logs</a>
    </div>
@endsection
